...
The environment rewards correct classifications and penalizes incorrect predictions, enabling **systematic evaluation of AI agents**.
...
| Task   | Description                                        |
| ------ | -------------------------------------------------- |
| easy   | Clear merchant names                               |
| medium | Diverse merchants                                  |
| hard   | Ambiguous merchants and mixed transaction patterns |

The harder tasks require **context reasoning using merchant and transaction amount**.

Example:

| Merchant | Amount | Category      |
| -------- | ------ | ------------- |
| Apple    | 45000  | shopping      |
| Apple    | 120    | entertainment |

Agents must reason about **transaction context**, not just merchant name.
...
Amazon, Flipkart, Myntra, BigBazaar, RelianceMart
...
This merchant coverage allows agents to **reason about transaction categories using real-world brand signals**.
...
A simple baseline agent is implemented in **`inference.py`**.

It uses rule-based merchant matching to classify transactions.
